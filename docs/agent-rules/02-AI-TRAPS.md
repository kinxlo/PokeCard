...
await db.inventory.update({ /* decrement stock */ });
// If second call fails, order exists but inventory not updated!

// ✅ CORRECT - Transaction
await db.$transaction(async (tx) => {
  await tx.order.create({ data: orderData });
  await tx.inventory.update({ /* decrement stock */ });
});
```
...
async function processOrder(order: Order) {
  ...
  try {
    ...
    logger.error('Order processing failed', { orderId: order.id, traceId, error: err });
    throw err;
  }
}
```
