...
export default function PokemonDetailPage({params}: PageProps) {
  ...
  return (
    <div className="flex-1 overflow-y-auto relative">
      ...
      <div className="max-w-360 mx-auto py-8 relative z-10">
        {/* Breadcrumb */}
        <span
          onClick={() => window.history.back()}
          className="inline-flex items-center hover:cursor-pointer gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors mb-8"
        >
          <ArrowLeft className="h-4 w-4"/>
          Back to All Pokémon
        </span>
        ...
        <div className="grid lg:grid-cols-2 gap-2">
          ...
          <div className="space-y-2">
            ...
            <div className={`relative bg-linear-to-br ${gradientClass} rounded-3xl overflow-hidden shadow-2xl group`}>
              ...
              <div
                className="absolute inset-0 bg-[radial-gradient(circle_at_20%_30%,rgba(255,255,255,0.15),transparent_50%)]"/>
              <div
                className="absolute inset-0 bg-[radial-gradient(circle_at_80%_70%,rgba(255,255,255,0.1),transparent_50%)]"/>

              {/* Pokemon Number Watermark */}
              <span className="absolute top-8 right-8 text-9xl font-black text-white/10 pointer-events-none"
                    aria-hidden="true">
                    #{pokemon.id.toString().padStart(3, '0')}
                  </span>
              ...
              <div className="absolute bottom-6 right-6 flex gap-3 z-20">
                <button
                  className="p-3 rounded-full bg-white/25 hover:bg-white/40 backdrop-blur-md shadow-lg transition-all hover:scale-110">
            ...
            <div className="grid grid-cols-3 gap-2">
              <div
                className="bg-card/80 backdrop-blur-md border rounded-2xl p-5 shadow-sm hover:shadow-md transition-all hover:-translate-y-1">
            ...
            {/* Abilities */}
            <div
              className="bg-card/80 backdrop-blur-md border rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="space-y-3">
                {pokemon.abilities.map(({ability, is_hidden}) => (
                  ...
                  >
                    ...
                    {is_hidden && (
                      <span
                        className="text-[11px] px-3 py-1.5 rounded-lg bg-purple-500 text-white font-bold uppercase tracking-wider shadow-sm">
                            Hidden
                          </span>
                    )}
                  </div>
                ...
                {pokemon.stats.map(({stat, base_stat}) => (
                  <div key={stat.name}>
                    <div className="flex items-center justify-between mb-2">
                      <span
                        className="text-sm font-bold min-w-28 uppercase tracking-wide">{formatStatName(stat.name)}</span>
            ...
            <div
              ...
              <div className="max-h-96 overflow-y-auto pr-2 custom-scrollbar">
                ...
                {pokemon.moves.length > 40 && (
                  <div
                    className="text-sm text-muted-foreground mt-4 text-center py-3 bg-muted/30 rounded-xl font-semibold">
